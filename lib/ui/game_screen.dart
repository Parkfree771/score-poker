...
class _GameScreenState extends State<GameScreen> with TickerProviderStateMixin {
    ...
    if (widget.initialState != null) {
      ...
      if (state.phase == GamePhase.playing && widget.persona != null) {
        ...
        WidgetsBinding.instance.addPostFrameCallback(
            (_) => _personaSay(widget.persona!.lines.greeting));
      }
    } else {
      _start();
    }
  ...
  void _start() {
    state = GameState.deal(seed: widget.seed);
    selected = null;
    _flyingHandIndex = null;
    _animating = false;
    _recorded = false;
      ...
      body: DecoratedBox(
        ...
        child: SafeArea(
          child: state.phase == GamePhase.awaitingReveal
              ...
              : Stack(
                  children: [
                    landscape ? _landscapeLayout(l10n) : _portraitLayout(l10n),
                    // 이모트 피커: 바깥을 탭하면 닫힘
                    if (_emoteOpen) ...[
                      Positioned.fill(
                    ...
                    if (_oppSpeech != null)
                      Positioned(
                        ...
                        child: Align(
                          ...
                          child: _SpeechBubble(
                              key: ValueKey('sp-$_oppSpeech'), text: _oppSpeech!),
                        ),
                      ),
  ...
  Widget _landscapeLayout(AppLocalizations l10n) => Column(
        children: [
          SizedBox(height: 88, child: _lsTopRow(l10n)),
          Expanded(child: _tableArea(landscape: true)),
          SizedBox(height: 104, child: _lsBottomRow(l10n)),
        ],
      );
  ...
  Widget _lsBottomRow(AppLocalizations l10n) {
    final wins = _lineWins();
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(child: _handBar(l10n, height: 100)),
          _emoteButton(l10n),
      ...
      child: Row(
        ...
        children: [
          ...
          Text('${state.deckRemaining}',
              style: const TextStyle(color: AppColors.textMuted, fontWeight: FontWeight.w800, fontSize: 13)),
        ],
      ),
    );
  }
  ...
  Widget _tableArea({required bool landscape}) => Padding(
        ...
        child: DecoratedBox(
          ...
          child: Stack(
            children: [
              const Positioned.fill(child: CustomPaint(painter: TableDecorPainter())),
                    ...
                    : Row(
                        children: [
                          SizedBox(
                            width: 66,
                            child: Center(
                              child: DeckPileView(remaining: state.deckRemaining, pileKey: _deckKey),
                            ),
                          ),
                          Expanded(child: _boardView(landscape: false)),
                          SizedBox(
                            width: 66,
                            child: Center(child: DiscardPileView(cards: _grave, pileKey: _graveKey)),
                          ),
                        ],
                      ),
              ),
        ...
        children: [
          ...
          Flexible(
            child: Text(widget.persona?.name ?? l10n.player2,
                ...
                style: const TextStyle(color: AppColors.textMuted, fontWeight: FontWeight.w700, fontSize: 13)),
          ),
  ...
  Widget _emoteButton(AppLocalizations l10n) => OutlinedButton(
        onPressed: () => setState(() => _emoteOpen = !_emoteOpen),
        style: OutlinedButton.styleFrom(
          padding: const EdgeInsets.all(10),
          minimumSize: Size.zero,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          side: BorderSide(color: AppColors.gold.withValues(alpha: _emoteOpen ? 1 : 0.55), width: 1.4),
        ),
  ...
  Widget _foldButton() => OutlinedButton(
  ...
  Widget _handBar(AppLocalizations l10n, {required double height}) {
    ...
    _handKeys = [for (var i = 0; i < hand.length; i++) GlobalKey()];
    final dim = !_myTurn; // 내 차례 아니면 흐리게(대신 판은 안 뒤집힘)
    return SizedBox(
      key: _handAreaKey,
      height: height,
      child: hand.isEmpty
          ? const Center(child: Text('—', style: TextStyle(color: AppColors.textMuted, fontSize: 20)))
          : Center(
              child: FittedBox(
      ...
      if (owner == me) {
        ...
        if (card.isJoker) {
          ...
          await _placeWithFly(
            ...
            col: target,
            displayCard: display,
            commit: () => state.placeCard(handIndex, owner, row, target, jokerRank: picked.$1, jokerSuit: picked.$2),
          );
        } else {
          await _placeWithFly(
            handIndex: handIndex,
        ...
        if (cell == null) {
          if (card.isShield) {
            ...
            _snack('상대 빈 칸에는 쉴드만 놓을 수 있습니다');
          }
        } else {
          await _removeWithFly(handIndex, owner, row, col, card);
        }
      }
  ...
  /// 같은 숫자/조커로 상대 카드를 **쳐내는** 연출: 무기 카드가 날아가 타격 →
  /// 맞은 카드가 튕겨 오른 뒤 **빙글 돌며 무덤으로 날아감** →
  /// 보상으로 **새 쉴드 카드를 덱에서 손패로 드로우**하는 연출 → 턴 종료(AI 진행).
  Future<void> _removeWithFly(int handIndex, PlayerId owner, int row, int col, PlayingCard weapon) async {
    ...
    if (from == null || cellRect == null || victim == null) {
      state.removeOpponentCard(handIndex, row, col);
      if (victim != null) _grave.add(victim);
    ...
    // 1) 무기 카드가 타격 지점으로 돌진.
    await flyCard(overlay: overlay, vsync: this, from: from, to: cellRect, card: weapon, duration: const Duration(milliseconds: 300));
    if (!mounted) return;
    // 2) 규칙 반영: 카드 제거 + 보상 쉴드가 손패로 드로우됨(턴 종료).
    state.removeOpponentCard(handIndex, row, col);
    setState(() => _flyingHandIndex = null);
    // 3) 맞은 카드가 튕겨 나간 뒤 무덤으로 회전 비행.
    await poofCard(overlay: overlay, vsync: this, rect: cellRect, card: victim, duration: const Duration(milliseconds: 220));
    if (!mounted) return;
    ...
    setState(() => _grave.add(victim));
    // 4) 보상 카드가 덱 → 내 손패로 날아드는 드로우 연출.
    final reward = state.lastReward;
    final deckRect = _rectFor(_deckKey);
    final handRect = _rectFor(_handAreaKey);
    if (reward != null && deckRect != null && handRect != null) {
      await flyCard(overlay: overlay, vsync: this, from: deckRect, to: handRect, card: reward, duration: const Duration(milliseconds: 360));
    }
    if (!mounted) return;
    setState(() {
      _animating = false;
    ...
    try {
      switch (move) {
        ...
        case PlaceMove(:final handIndex, :final target, :final row, :final col, :final jokerRank, :final jokerSuit):
          ...
          final display =
              card.isJoker ? card.designate(jokerRank!, jokerSuit!) : card;
          final from = _rectFor(_oppHandKey);
          final to = _rectFor(_boardKey(target, row, col));
          if (from != null && to != null) {
            await flyCard(overlay: Overlay.of(context), vsync: this, from: from, to: to, card: display);
    ...
    return showDialog<(int, Suit)>(
      ...
      builder: (context) => AlertDialog(
          ...
          builder: (context, setLocal) => Column(
            ...
            children: [
              DropdownButton<int>(
                isExpanded: true,
                value: rank,
                items: [for (final r in Ranks.all) DropdownMenuItem(value: r, child: Text('${l10n.numberLabel}: ${rankLabel(r)}'))],
                onChanged: (v) => setLocal(() => rank = v ?? rank),
              ),
              DropdownButton<Suit>(
                isExpanded: true,
                value: suit,
                items: [for (final s in Suit.values) DropdownMenuItem(value: s, child: Text('${l10n.suitLabel}: ${s.symbol}'))],
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.pop(context, (rank, suit)), child: Text(l10n.confirm)),
        ],
      ),
    );
  }
    ...
    final resultHead = Column(
      ...
      children: [
        const Text('GAME OVER',
            style: TextStyle(
                fontSize: 12, color: AppColors.textMuted, letterSpacing: 3, fontWeight: FontWeight.w700)),
  ...
  Widget _resultLine(AppLocalizations l10n, int i) {
    ...
    Widget side(HandResult e, {required bool winner, required bool alignRight}) {
      final texts = Column(
        ...
        children: [
          Text(_categoryName(l10n, e.category),
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
              style: TextStyle(
  ...
  String _categoryName(AppLocalizations l10n, HandCategory c) => switch (c) {
  ...
  Widget _scrim({required Widget child, bool compact = false}) => Container(
        ...
        child: Container(
          ...
          child: SingleChildScrollView(
            padding: EdgeInsets.symmetric(horizontal: compact ? 22 : 28, vertical: compact ? 14 : 32),
            child: child,
          ),
        ),
      );
    ...
    return IgnorePointer(
      child: TweenAnimationBuilder<double>(
        ...
        builder: (context, t, child) =>
            Opacity(opacity: t.clamp(0.0, 1.0), child: Transform.scale(scale: 0.6 + 0.4 * t, child: child)),
        child: Container(
  ...
  Widget build(BuildContext context) {
    return IgnorePointer(
      child: TweenAnimationBuilder<double>(
        ...
        curve: Curves.easeOutBack,
        builder: (context, t, child) =>
            Opacity(opacity: t.clamp(0.0, 1.0), child: Transform.scale(scale: 0.7 + 0.3 * t, child: child)),
        child: Container(
