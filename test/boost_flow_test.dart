...
/// 부스트 판을 화면에서 실제로 눌러 본다: 칩 5개, 스왑 버튼 → 손패가 전부 새로 온다,
...
void main() {
  ...
  Future<void> drain(WidgetTester tester) => pumpFor(
      tester, const Duration(seconds: 800), step: const Duration(milliseconds: 500));
  ...
  testWidgets('부스트 판: 칩 5개 + 스왑 1회', (tester) async {
    ...
    // 내 레일에 칩 5개(파랑 링), 상대는 4개.
    final mine = tester
        .widgetList<VeilChip>(find.byType(VeilChip))
        .where((c) => c.filled)
        .length;
    expect(mine, greaterThanOrEqualTo(9), reason: '내 5 + 상대 4 = 채워진 칩 9개');

    expect(labeled('hand-'), findsNWidgets(5));
    await tester.tap(find.text(l10n.swapButton));
    await tester.pump();
    await pumpFor(tester, const Duration(seconds: 3));
    expect(labeled('hand-'), findsNWidgets(5), reason: '스왑 뒤에도 손패는 5장');
  ...
  testWidgets('카드를 한 장 놓으면 스왑 버튼이 사라진다', (tester) async {
    ...
    await tester.tap(labeled('hand-$idx'));
    await tester.pump();
